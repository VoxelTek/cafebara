...
function statusSettings:draw(onFocus)
    ...
    elseif STORMBREAKER.isConnected() then
        local chargeStatus = STORMBREAKER.getChargeStatus()

        self.menuSystem:printLine("Stormbreaker detected")


        if chargeStatus == STORMBREAKER.CHG_STAT_NOT_CHG then
            self.menuSystem:printLine("Not charging")
        elseif chargeStatus == STORMBREAKER.CHG_STAT_PRE_CHG or chargeStatus == STORMBREAKER.CHG_STAT_FAST_CHG then
            self.menuSystem:printLine("Charging")
        else
            self.menuSystem:printLine("Charging complete")
        end
        self.menuSystem:printLine(string.format("Battery SOC: %.1f", STORMBREAKER.getSOC()) .. "%%")
        self.menuSystem:printLine(string.format("Battery voltage: %.0f mV", STORMBREAKER.getVCell()))
    end
